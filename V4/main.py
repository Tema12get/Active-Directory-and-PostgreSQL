...
class UserManagementWindow(QWidget):
    ...
    def __init__(self, ad_domain: str, ad_admin_user: str, ad_admin_password: str,
        ...
        self.ad_use_ssl = ad_use_ssl  # Флаг для использования SSL в AD
        self.current_db = None  # Текущая выбранная база данных PostgreSQL
    ...
    def connect_to_ad(self, use_ssl: bool = None) -> ldap3.Connection:
        """
        Создает подключение к Active Directory.

        Args:
            use_ssl (bool): Использовать SSL для подключения. Если None, использует self.ad_use_ssl.

        Returns:
            ldap3.Connection: Объект подключения к AD или None в случае ошибки.
        """
        if use_ssl is None:
    ...
    def connect_to_postgres(self, dbname: str = "postgres") -> psycopg2.extensions.connection:
        ...
        Args:
            dbname (str): Имя базы данных (по умолчанию "postgres").

        Returns:
            psycopg2.extensions.connection: Объект подключения к PostgreSQL или None в случае ошибки.
        """
        try:
    ...
    def _build_ui(self):
        ...
        Включает секции для Active Directory, PostgreSQL и кнопок действий.
        """
        ...
        # --- Подгруппа списка пользователей PostgreSQL ---
    ...
    # --- Загрузка баз данных PostgreSQL ---
    def load_postgres_databases(self):
        """
        Загружает список доступных баз данных PostgreSQL и заполняет выпадающий список.
        """
        try:
    ...
    def on_database_changed(self):
        """
        Обработчик смены выбранной базы данных, обновляет список пользователей PostgreSQL.
        """
        self.current_db = self.database_selector.currentText()
        logging.info("Выбрана база данных: %s", self.current_db)
        self.load_postgres_users()

    # --- Загрузка пользователей PostgreSQL ---
    def load_postgres_users(self):
        """
        Загружает список пользователей PostgreSQL с правом входа из текущей базы
    ...
    def on_postgres_user_selected(self):
        """
        Обработчик выбора пользователя PostgreSQL из списка.
        Заполняет поле логина данными выбранного пользователя.
        """
    ...
    # --- Загрузка организационных единиц Active Directory ---
    def load_ad_organizational_units(self):
        """
        Загружает организационные единицы (OU) из Active Directory
        и заполняет выпадающий список.
        """
        try:
    ...
    def load_ad_users_in_ou(self):
        """
        Загружает список пользователей из выбранной OU в Active Directory
        и отображает их в списке.
        """
        try:
    ...
    def on_ad_user_selected(self):
        """
        Обработчик выбора пользователя AD из списка.
        Заполняет поля логина и отображаемого имени данными выбранного пользователя.
        """
    ...
    def on_create_clicked(self):
        """Обработчик кнопки 'Создать'. Вызывает создание пользователя."""
        try:
            self.create_user()
        except Exception as error:
    ...
    def on_modify_clicked(self):
        """Обработчик кнопки 'Изменить пароль'. Вызывает изменение пароля."""
        try:
    ...
    def on_delete_clicked(self):
        """Обработчик кнопки 'Удалить'. Вызывает удаление пользователя."""
        try:
            self.delete_user()
        except Exception as error:
    ...
    def create_user(self):
        """
        Создает нового пользователя в Active Directory и PostgreSQL.
        Проверяет валидность данных и наличие пользователя перед созданием.
        """
    ...
    def modify_password(self):
        """
        Изменяет пароль пользователя в Active Directory и PostgreSQL, если он существует.
        """
        login = self.login_input.text().strip()
        password = self.password_input.text()
        confirm_password = self.password_confirm_input.text()
    ...
    def delete_user(self):
        """
        Удаляет пользователя из Active Directory и PostgreSQL, если он существует.
        """
        login = self.login_input.text().strip()
        if not login:
    ...
    # --- Операции с Active Directory ---
    def check_ad_user_exists(self, login: str) -> bool:
        """
        Проверяет, существует ли пользователь в Active Directory.

        Args:
            login (str): Логин пользователя (sAMAccountName).
    ...
    def create_ad_user(self, display_name: str, login: str, password: str, ou_distinguished_name: str):
        """
        Создает нового пользователя в Active Directory с указанными параметрами.
        При ошибке установки пароля удаляет созданный объект для предотвращения неполноценных учётных записей.

        Args:
            display_name (str): Отображаемое имя пользователя (CN).
            login (str): Логин пользователя (sAMAccountName).
            password (str): Пароль пользователя.
            ou_distinguished_name (str): DN организационной единицы, где создается пользователь.
        """
        connection = self.connect_to_ad()
        user_dn = f"CN={display_name},{ou_distinguished_name}"
        # Биты userAccountControl
        UAC_NORMAL = 0x0200
        ...
        # Предупреждение, если SSL отключён
        ...
        try:
            if not connection.extend.microsoft.modify_password(user_dn, password):
                error = connection.result
                # Удаляем пользователя при ошибке установки пароля
                try:
    ...
    def update_ad_password(self, login: str, password: str):
        """
        Обновляет пароль пользователя в Active Directory.
    ...
    def check_postgres_user_exists(self, login: str, host: str) -> bool:
        """
        Проверяет, существует ли пользователь в PostgreSQL.

        Args:
            login (str): Имя пользователя PostgreSQL.
    ...
    def create_postgres_user(self, login: str, password: str, host: str, roles: List[str]):
        """
        Создает нового пользователя в PostgreSQL и назначает ему роли, если указаны.

        Args:
            login (str): Имя пользователя PostgreSQL.
            password (str): Пароль пользователя.
            host (str): Хост PostgreSQL сервера.
            roles (List[str]): Список ролей для назначения пользователю.
        """
        try:
            with self.connect_to_postgres(self.current_db) as connection:
                with connection.cursor() as cursor:
    ...
    def update_postgres_password(self, login: str, password: str, host: str):
        """
        Обновляет пароль пользователя в PostgreSQL.
...
class LoginWindow(QWidget):
    """
    Окно входа для ввода учетных данных Active Directory и PostgreSQL.
    Проверяет подключение к обоим сервисам перед открытием главного окна.
    """
    def __init__(self):
    ...
    def _build_ui(self):
        """Создает и настраивает пользовательский интерфейс окна входа."""
        main_layout = QVBoxLayout()
        ...
        # Чекбокс для использования SSL в AD
        self.ad_ssl_checkbox = QCheckBox("Использовать SSL для подключения к AD")
        self.ad_ssl_checkbox.setChecked(True)  # По умолчанию включено
    ...
    def on_continue(self):
        """
        Обработчик кнопки 'Продолжить'. Проверяет подключение к AD и PostgreSQL
        и открывает главное окно при успешной проверке.
        """
        ...
        # Проверка подключения к AD с учетом флага SSL
        try:
        ...
        # Открытие главного окна при успешной проверке
    ...
    def open_main_window(self, ad_domain: str, ad_admin_user: str, ad_admin_password: str,
        ...
        """
        Открывает главное окно приложения и закрывает окно входа.

        Args:
            ad_domain (str): Домен Active Directory.
            ad_admin_user (str): Имя администратора AD.
            ad_admin_password (str): Пароль администратора AD.
            postgres_host (str): Хост PostgreSQL сервера.
            postgres_admin_user (str): Имя администратора PostgreSQL.
            postgres_admin_password (str): Пароль администратора PostgreSQL.
            ad_use_ssl (bool): Флаг использования SSL для AD.
        """
...
if __name__ == "__main__":
    """
    Запускает приложение, создает окно входа и начинает основной цикл обработки событий.
    """
